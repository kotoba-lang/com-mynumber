(ns mynumber.main
  "Kotodama WASM entrypoint for the Mynumber clean-room actor (L4) — Clojure port.
  ...
  facts (`mynumber.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mynumber")
(def tier "L4")
...
(def entity-specs
  [{:entity "Filing"       :plural "filings"       :id-prefix "mynumber_fil"
    :fields [:filerId :form :fiscalYear :status]    :required [:form :fiscalYear]
    :coerce {}                       :refs {}}
   {:entity "Corporation"  :plural "corporations"  :id-prefix "mynumber_cor"
    :fields [:name :corporateNumber :jurisdiction] :required [:name :corporateNumber]
    :coerce {}                       :refs {}}
   {:entity "Disclosure"   :plural "disclosures"   :id-prefix "mynumber_dis"
    :fields [:corporationId :docType :submittedAt] :required [:docType :submittedAt]
    :coerce {}                       :refs {:corporationId "Corporation"}}
   {:entity "Citizen"      :plural "citizens"      :id-prefix "mynumber_cit"
    :fields [:myNumber :municipality]              :required [:myNumber :municipality]
    :coerce {}                       :refs {}}
   {:entity "Application"  :plural "applications"  :id-prefix "mynumber_app"
    :fields [:citizenId :procedure :status]        :required [:procedure :status]
    :coerce {}                       :refs {:citizenId "Citizen"}}
   {:entity "Dataset"      :plural "datasets"      :id-prefix "mynumber_dat"
...
(defn emit-facts
  "EAVT facts for one record: {\"mynumber.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mynumber-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mynumber-compat/ready)
